...
class MagicEightBall extends React.Component {
  ...
  <label htmlFor="leftnum">
      First Plant # <input id="leftnum" type="number" value={this.state.minPlant} max={totalPlants} min="1" step="10" onChange={(event)=>this.slideleft(event)} onInput={(event)=>this.slideleft(event)}/> 
  </label>
  <label htmlFor="rightnum">
     Last Plant # <input id="rightnum" type="number" value={this.state.maxPlant} max={totalPlants} min="10" step="10" onChange={(event)=>this.slideright(event)} onInput={(event)=>this.slideright(event)}/>
  </label>
            </div>
